test_that('Test SeqCountData.', {
    gexp <- read.table(system.file(package = 'hespresso', 'extdata', 'C_flexuosa.tsv.gz'), header = TRUE, sep = '\t', row.names = 1)
    group <- c('wet', 'wet', 'wet', 'dry', 'dry', 'dry')
    hnames <- read.table(system.file(package = 'hespresso', 'extdata', 'C_flexuosa.homeolog.tsv.gz'), header = TRUE, sep = '\t')
    x <- newSeqCountData(gexp, group, hnames)
    show(x)
})


test_that('Test SeqCountData combine.', {
...
test_that('Test SeqCountData subset', {
