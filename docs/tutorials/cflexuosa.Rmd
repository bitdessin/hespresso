...
In this example, we demonstrate how to use HOBIT and HomeoRoq
...
which links gene expression to their corresponding homeologs.
...
The plot generated by the `plot_HER()` function can also be visualized using plotly,
...
x_homeoroq$pvalue[is.na(x_homeoroq$pvalue)] <- 1
x_homeoroq$qvalue[is.na(x_homeoroq$qvalue)] <- 1
```
