...
In this tab-separated table, each row represents one homeolog triad,
...
We then construct an `SeqCountData` class object using the `newSeqCountData()` function.
...
x_DvsAB <- combine_hexp(x, subgenomes = c(1, 2))
x_DvsAB
```

In this case, the combined A- and B-subgenomes are appended as the last subgenome,
so the subgenome order in `x_DvsAB` becomes D and A+B.
...
as D-subgenome is now the first subgenome in `x_DvsAB`.
...
distr_plots_AB <- plot_HER_distr(x_DvsAB)
...
The resulting distributions show a peak around one-third,
...
x_DvsAB_output <- hobit(x_DvsAB)
x_DvsAB_output$pvalue[is.na(x_DvsAB_output$pvalue)] <- 1
x_DvsAB_output$qvalue[is.na(x_DvsAB_output$qvalue)] <- 1
```

```{r tt-wheat-combine-subgenomes-hobit-output}
head(x_DvsAB_output[order(x_DvsAB_output$pvalue), ])
```
...
is_sig <- ifelse(x_DvsAB_output$qvalue < 0.01, "q<0.01", "q>=0.01")
plot_HER(x_DvsAB, label = is_sig)
```
