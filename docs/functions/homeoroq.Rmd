...
and repeats the calculation four times (`chains = 4`).
...
The `homeoroq()` function requires an input object of class `SeqCountData`,
as does the `hobit()` function.
The original implementation of HomeoRoq requires and evaluates FPKM [@ref_homeoroq].
For explanatory purposes, we use raw read counts here.


```{r fc-hq-input-newSeqCountData}
gexp <- read.table("data/c_flexuosa.0516.mini.txt.gz",
                   header = TRUE, sep = "\t", row.names = 1)

# Normalize gexp to FPKM to reproduce the HomeoRoq analysis.
...
Once an `SeqCountData` object is prepared,
run the test using the `homeoroq()` function.
