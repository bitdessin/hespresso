...
- `pvalue`: *p*-value from the LRT based on normalized likelihoods. `NA` indicates that the homeolog is expressed in only one condition, making ratio comparisons infeasible and consequently preventing the calculation of *p*-value.
...
Note that all statistics are derived from MCMC samples and
...
By default, HOBIT assumes that homeolog expression follows an NB distribution.
...
To model homeolog expression,
user can set `dist = "ZINB"`
...
For sampling homeolog expression ratios,
HOBIT supports using a Dirichlet prior distribution instead of NIP.
