...
abstract: |
    ...
    under controlled conditions. The _hespresso_ package includes a suite of functions
...
The hespresso package provides functions
...
The output is a list of data.frame objects, with each element corresponding to a condition group.
Each data.frame contains the true mean expression values used for sampling,
...
and input them into the `auc()` and `roc()` functions provided by the pROC package.
