...
by regulating **homeolog expression ratios** (**HERs**) in response to developmental and environmental changes
...
exhibited HER shifts under cold and zinc stress [@ref_homeoroq; @ref_kamzinc].
...
In another case, _Cardamine insueta_ (2*n* = 3*x* = 24, RRA),
a triploid resulting from hybridization between _Cardamine rivularis_ (2*n* = 2*x* = 16, RR) and _C. amara_
modulates HERs in response to submergence stress [@ref_cins_submergestress].

The `r Biocpkg("hespresso")` package is designed to detect homeologs with shifted expression ratios,
referred to as **ratio-shifted homeologs** (**RSHs**),
across diverse conditions using RNA-Seq data from allopolyploids.
It provides two methods for RSH detection, **HOBIT** [@ref_hobit] and **HomeoRoq** [@ref_homeoroq].
...
`r Biocpkg("hespresso")` equips researchers with the statistical power and flexibility
needed for homeolog expression analysis.
Take a sip of `r Biocpkg("hespresso")` and begin your analysis with confidence.


# Installation

To install the `r Biocpkg("hespresso")` package,
...
BiocManager::install("hespresso")
```

Note that to install the latest version of `r Biocpkg("hespresso")`
...
The dispersion parameter $\phi$ is estimated using the `estimateDisp()` function from the `r Biocpkg("edgeR")` package.
...
provided in the `r Biocpkg("hespresso")` package.
...
by internally calling the `r Biocpkg("edgeR")` package.
However, users may choose to perform normalization using other appropriate
packages such as `r Biocpkg("edgeR")` or `r Biocpkg("DESeq2")`.
...
The result (`x_output`) is a DataFrame object (defined in the `r Biocpkg("S4Vectors")` package),
with one row per homeolog.
...
and the dispersion parameter is estimated using the `estimateDisp()` function from the `r Biocpkg("edgeR")` package.
...
set `chains = 10` and `iter_sampling = 1e4`.
...
The result (`x_output`) is a DataFrame class object with one row per homeolog,
...
- **Case Studies** (`casestudies`): Step-by-step tutorials for analyzing real RNA-Seq datasets using the `r Biocpkg("hespresso")` package.
...
To list all available vignettes included in the `r Biocpkg("hespresso")` package,
run as below.
